...
class Tara_SmallFavorEnd_Patch
{
    ...
    {
        return GenericItemPatcher.Transpiler(instructions, il, [0, 200]);
    }
}
    ...
    static bool Prefix(CarolineCasinoSpawn __instance)
    {
        if (Archipelago.Enabled)
        {
            if (__instance.SpawnLoc == CarolineCasinoSpawn.Loc.Jail)
            {
                ...
                {
                    AccessTools.Method(typeof(CarolineCasinoSpawn), "SET").Invoke(__instance, [new Vector2(608, -48)]);
                }
                return false;
            }
            ...
            if (__instance.SpawnLoc == CarolineCasinoSpawn.Loc.Boss)
            {
                ...
                {
                    AccessTools.Method(typeof(CarolineCasinoSpawn), "SET").Invoke(__instance, [new Vector2(976, -464)]);
                }
                return false;
            }
        }
