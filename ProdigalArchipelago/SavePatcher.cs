        ...
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
        {
            ...
            foreach (CodeInstruction code in instructions)
            {
                if (!check_ap_file_patched && code.opcode == OpCodes.Call && (MethodInfo)code.operand == AccessTools.Method(typeof(Application), "get_persistentDataPath"))
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(string), nameof(String.Concat), new Type[] {typeof(string), typeof(string), typeof(string), typeof(string)}));
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(File), nameof(File.Exists), new Type[] {typeof(string)}));
                ...
                else if (check_ap_file_patched && !check_file_patched && code.opcode == OpCodes.Call && (MethodInfo)code.operand == AccessTools.Method(typeof(File), nameof(File.Exists), new Type[] {typeof(string)}))
                {
                    yield return new CodeInstruction(OpCodes.Stloc, local_filename);
                    yield return new CodeInstruction(OpCodes.Ldloc, local_filename);
                    yield return code;
                ...
                else if (check_file_patched && !load_file_patched && code.opcode == OpCodes.Newobj)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(File), nameof(File.Open), new Type[] {typeof(string), typeof(FileMode)}));
                    ...
                    yield return new CodeInstruction(OpCodes.Callvirt, AccessTools.Method(typeof(BinaryFormatter), nameof(BinaryFormatter.Deserialize), new Type[] {typeof(Stream)}));
                    ...
                    yield return new CodeInstruction(OpCodes.Ldloc_1)
                    {
                        labels = new List<Label>() { label_load_normal_save }
                    };
                    yield return new CodeInstruction(OpCodes.Callvirt, AccessTools.Method(typeof(BinaryFormatter), nameof(BinaryFormatter.Deserialize), new Type[] {typeof(Stream)}));
